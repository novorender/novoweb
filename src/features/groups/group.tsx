...
export function Group(
  { group, disabled }: { group: ObjectGroup; disabled: boolean },
) {
  ...
  return (
    <>
        ...
        <Box display="flex" width={1} alignItems="center">
          ...
          <Box flex="0 0 auto">
            <StyledCheckbox
              ...
              icon={
                <Visibility
                  htmlColor={`rgba(${r}, ${g}, ${b}, ${Math.max(a ?? 0, 0.2)})`}
                />
              }
              checkedIcon={!group.opacity
      ...
      <ColorPicker
        ...
        onChangeComplete={({ rgb }) =>
          dispatchObjectGroups(
            objectGroupsActions.update(group.id, { color: rgbToVec(rgb) }),
          )}
      />
      <Menu
        onClick={(e) => e.stopPropagation()}
          ...
          <Route path={match.path} exact>
            {(isAdmin
              ? [
                ...
                <MenuItem
                  key="delete"
                  onClick={() => history.push("/delete/" + group.id)}
                >
                  <ListItemIcon>
                    <Delete fontSize="small" />
                  </ListItemIcon>
              ...
              : []).concat(
                ...
                <MenuItem
                  key="opacity"
                  onClick={() => history.replace(match.path + "/opacity")}
                >
                  <ListItemIcon>
                    <Opacity fontSize="small" />
                  </ListItemIcon>
          ...
          <Route path={match.path + "/opacity"} exact>
            {[0, 0.25, 0.5, 0.75].map((opacity) => (
              <MenuItem
                ...
                onClick={() => {
                  dispatchObjectGroups(
                    objectGroupsActions.update(group.id, { opacity }),
                  );
                }}
              >
                <ListItemText>{(1 - opacity) * 100}%</ListItemText>
