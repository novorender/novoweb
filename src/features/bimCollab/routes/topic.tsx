...
import {
  selectionBasketActions,
  useDispatchSelectionBasket,
} from "contexts/selectionBasket";
import {
  CameraType,
  ObjectVisibility,
  renderActions,
...
import {
  translateBcfClippingPlanes,
  translateOrthogonalCamera,
  translatePerspectiveCamera,
} from "utils/bcf";
...
export function Topic() {
  ...
  const floatingViewpoints: Comment[] = viewpoints
    ?.filter((vp) =>
      !comments.some((comment) => comment.viewpoint_guid === vp.guid)
    )
    .map((floatingVp) => ({
      guid: "",
  ...
  const topicActions = topic.authorization?.topic_actions ??
    extensions?.topic_actions ?? [];
  const topicStatuses = topic.authorization?.topic_status ??
    extensions?.topic_status ?? [];
  ...
  return (
    <>
      ...
      <Box boxShadow={theme.customShadows.widgetHeader}>
        ...
        {topicActions.includes("createComment")
          ? (
            <Button
              component={Link}
              to={`/project/${projectId}/topic/${topicId}/new-comment`}
              color="grey"
            >
              <Add sx={{ mr: 1 }} />
              Add comment
        ...
        {topicStatuses.length
          ? (
            <Button
              component={Link}
              to={`/project/${projectId}/topic/${topicId}/edit`}
              color="grey"
            >
      ...
      <ScrollBox height={1} width={1} horizontal sx={{ mt: 1 }}>
        ...
        >
          {thumbnail
            ? <img onClick={() => toggleModal()} src={thumbnail} alt="" />
            : null}
          <Accordion>
            <AccordionSummary>
            ...
            <AccordionDetails sx={{ px: 1, pb: 2 }}>
              ...
              <Divider
                sx={{ mt: 1.5, mb: 1, color: theme.palette.grey[200] }}
              />
              <Box display="flex" justifyContent="space-between">
              ...
              <Box display="flex" justifyContent="space-between">
                ...
                <Typography variant="h6" fontWeight={600}>
                  {topic.due_date
                    ? new Date(topic.due_date).toLocaleDateString("nb")
                    : "Undecided"}
                </Typography>
              </Box>
              <Divider
...
function CommentListItem({
  ...
  const handleClick = async () => {
    ...
    const getColorGroups = coloring && coloring.length
      ? Promise.all(
        coloring
          .map((item) => ({
            ...item,
    ...
    const [visibilityExceptionIds, selectionIds, colorGroups] = await Promise
      .all([
    ...
    if (visibility?.default_visibility) {
      ...
      dispatch(
        renderActions.setDefaultVisibility(ObjectVisibility.Transparent),
      );
      dispatchHidden(hiddenActions.setIds([]));
      dispatchSelectionBasket(
        selectionBasketActions.set(visibilityExceptionIds),
      );
    }
    ...
    if (viewpoint?.perspective_camera) {
      ...
      dispatch(
        renderActions.setCamera({ type: CameraType.Pinhole, goTo: camera }),
      );
    }
...
async function guidsToIds(
  { guids, db, abortSignal }: {
    guids: string[];
    db: ObjectDB;
  ...
  const callback = (
    refs: HierarcicalObjectReference[],
  ) => (ids = ids.concat(extractObjectIds(refs)));
  for (let i = 0; i < batches.length / concurrentRequests; i++) {
    await Promise.all(
      batches.slice(
        i * concurrentRequests,
        i * concurrentRequests + concurrentRequests,
      ).map((batch) => {
