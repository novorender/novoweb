...
const initialState = {
    points: [[]] as vec3[][],
    height: [undefined] as (undefined | { min: number; max: number })[],
};
...
export const zoneSelectorSlice = createSlice({
    ...
    reducers: {
        setHeight: (state, action: PayloadAction<{ height: { min: number; max: number }; index: number }>) => {
            state.height[action.payload.index] = action.payload.height;
        },
...
export const selectZoneSelectorHeight = (state: RootState) => state.zoneSelector.height;
