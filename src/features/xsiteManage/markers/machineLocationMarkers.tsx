...
export const MachineLocationMarkers = forwardRef<{ update: () => void }>(function MachineLocationMarkers(_, ref) {
    const {
        state: { view },
    } = useExplorerGlobals();
    ...
    const update = useCallback(() => {
        if (!view?.measure || !containerRef.current.length || !locations.length) {
            return;
        }

        view.measure.draw.toMarkerPoints(locations.map((marker) => marker.position)).forEach((pos, idx) => {
