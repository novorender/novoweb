...
const PropertiesTree = lazy(() =>
  import("features/propertiesTree/propertiesTree")
);
...
const ClippingPlanes = lazy(() =>
  import("features/clippingPlanes/clippingPlanes")
);
...
const SelectionBasket = lazy(() =>
  import("features/selectionBasket/selectionBasket")
);
...
const HeightProfile = lazy(() =>
  import("features/heightProfile/heightProfile")
);
...
export function Widgets() {
  ...
  useEffect(
    function handleSettingsChange() {
      if (
        slots.length + (layout.widgets === 1 ? 0 : maximized.length) >
          layout.widgets
      ) {
  ...
  const getGridLayout = () => {
    if (layout.widgets === 4) {
      return {
        ...
        gridTemplateAreas: maximized.length === 2
          ? `"two one" "two one"`
          : maximized.length === 1
